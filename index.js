	...
	socket.on('ready_join', function(game_data){
		...
		for (var gameID in games){
			...
			if(String(gameID) == String(game_data.id) && games[gameID].joinable == true){
				...
				games[thisGameID].remaining_time = Math.round((games[thisGameID].remaining_distance_km / 0.00000000001));
				games[thisGameID].total_time = Math.round((games[thisGameID].remaining_distance_km / 0.00000000001));
